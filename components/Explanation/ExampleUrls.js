import { examplesContainer, button, buttonList, exampleTitle } from "./ExampleUrls.module.css";
...
const ExampleUrls = function( { url, handleUrlChange }) {
    ...
    return (
        <div className={examplesContainer}>
            ...
            {urls.map(url => <button key={url} className={button} onClick={handleClick}>{url}</button>)}
            </div>
        </div>
    )
}
